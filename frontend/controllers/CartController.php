...
class CartController extends CController {
    ...
    public function actionCheckout() {
        ...
        if ($model->load(Yii::$app->request->post()) && !empty(Yii::$app->session['checkout']) && Yii::$app->recaptcha->verifyResponse(
            ...
            $msg = 'Имя: ' . $model->name . '<br>E-mail: ' . $model->email . '<br>Телефон: ' . $model->phone . '<br>Сообщение: ' . $model->comment . '<br> Номер заказа: ' . $orderID . '<br> <a href="admin.fleshka.ru/checkout/' . $orderID . '">Ссылка на заказ</a>';
